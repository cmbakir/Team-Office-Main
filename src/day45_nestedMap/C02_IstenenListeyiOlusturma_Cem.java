...
public class C02_IstenenListeyiOlusturma_Cem extends SinifMapDepo {
       ...
       // {101={sinif=10, sube=H, soyisim=Can, Isim=Ali, bolum=MF},
        ...
        for(Integer eachKey:sinifKeySeti) {
            if(sinifMap.get(eachKey).get("sinif").equalsIgnoreCase("11")) {
                System.out.println(
                sinifMap.get(eachKey)+ " "+
                sinifMap.get(eachKey).get("sinif")+ " "+
                sinifMap.get(eachKey).get("sube")+ " "+
                sinifMap.get(eachKey).get("Isim")+ " "+ sinifMap.get(eachKey).get("soyisim"));
            }
        }

            // {sinif=11, sube=M, soyisim=Cem, Isim=Veli, bolum=Soz} 11 M Veli Cem
            //{sinif=11, sube=H, soyisim=Cem, Isim=Ali, bolum=TM} 11 H Ali Cem
            //{sinif=11, sube=M, soyisim=Cem, Isim=Sevgi, bolum=TM} 11 M Sevgi Cem
            //{sinif=11, sube=M, soyisim=Han, Isim=Esra, bolum=SOZ} 11 M Esra Han
    }
}
